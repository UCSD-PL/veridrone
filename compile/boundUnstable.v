  ...
  ((simpleBound triple triple2 PlusT 
     ...
     :: (simpleBound10 triple triple2 PlusT 
                      (premise triple /\ 
                       ...
                       ((ub triple + ub triple2)*(1+error) < floatMax) /\ 
                       (lb triple + lb triple2 >= R0)))
     :: (simpleBound11 triple triple2 PlusT 
                      (premise triple /\ 
                       premise triple2 /\
                       (ub triple + ub triple2 > (0 -floatMin)) /\
                       (lb triple + lb triple2 <= (0 -floatMin)) /\
                       (((0 - lb triple + (0 - lb triple2))*(1+error)) < floatMax)  /\ 
                       (ub triple + ub triple2 < R0)))
     :: List.nil). 
...
Definition combineTripleMinus (triple triple2 : singleBoundTerm) :=
  ((simpleBound2 triple triple2 MinusT 
     ...
     :: (simpleBound5 triple triple2 MinusT 
                      ...
                      :: (simpleBound8 triple triple2 MinusT 
                                       (premise triple /\ 
                                        premise triple2  /\ 
                                        (ub triple2 >= lb triple) /\ 
                                        (ub triple >= lb triple2) /\                                        (lb triple - ub triple2 < (0 - floatMin)) /\ (ub triple - lb triple2 > floatMin)  /\                                                               (ub triple - lb triple2 < floatMax) /\ 
                                        (ub triple2 - ub triple < floatMax) /\ 
                                        (ub triple2 - lb triple < floatMax) /\
                                        (lb triple - lb triple2 < floatMax))) :: List.nil)).   
...
Lemma denote_singleBoundTermProof : forall f st, denote_singleBoundTerm f (traceFromState st)=   (fun term : singleBoundTerm =>
      eval_formula (premise term) (traceFromState st) ->
      (eval_term (lb term) st st <= f <= eval_term (ub term) st st)%R).
      intros.
...
Proof.
  ...
  { simpl. 
    intros.
    rewrite <- denote_singleBoundTermProof. 
         ...
         (round_mode mode_NE) (x1 + x2))%R as roundedValue.
          ...
          }
          {
            destruct H0.
            {
              ...
              assert (floatMaxBound1 := H7).
              assert (resultGe0 := H9).
              clear H H1 H2 H3 H3 H6 H7 H9 IHexpr1 IHexpr2.
              unfold Semantics.eval_comp in *.
              simpl in *.
              remember (eval_term (lb x4) (hd tr) (hd (tl tr))) as lb1.
              remember (eval_term (lb x5) (hd tr) (hd (tl tr))) as lb2.
              remember (eval_term (ub x5) (hd tr) (hd (tl tr))) as ub2.
              remember (eval_term (ub x4) (hd tr) (hd (tl tr))) as ub1.
              clear Heqlb1 Heqlb2 Hequb1 Hequb2.
              destruct expr1Bound.
              destruct expr2Bound.
              assert (x1lb := H).
              assert (x1ub := H1).
              ...
              assert (x2ub := H3).
              clear H H1 H2 H3.
              destruct (Rle_dec (x1+x2)%R  (0 - floatMin)%R).
              {
                assert (floatMinCase3 := r0).
                clear r0.
                pose proof plusRoundingTruth3 as plusRoundingTruth3.
                specialize (plusRoundingTruth3 x x0 lb1 lb2 ub1 ub2 x1 x2 floatToRealRelationForExpr1 floatToRealRelationForExpr2).
                pose proof orExtra2 as orExtra.
                specialize (orExtra  (x1 + x2 >= floatMin)%R (x1 + x2 <= 0 - floatMin)%R floatMinCase3).
                specialize (plusRoundingTruth3 orExtra x1lb x2lb).
                Lemma ubMax : forall ub1 ub2, (ub1  + ub2 < 0 -> (ub1 + ub2) * (1+error) <floatMax)%R.
                  intros. 
                  unfold floatMin,floatMax in *.
                  pose proof custom_emaxGtCustom_emin.
                  pose proof bpow_lt.
                  SearchAbout (_>_ -> _<_)%Z.
                  apply Z.gt_lt in H0.
                  specialize (H1 radix2 custom_emin custom_emax H0).
                  pose proof errorLessThan1.
                  unfold error in *.
                  pose proof bpow_gt_0.
                  specialize (H3 radix2 (-custom_prec + 1)%Z ).
                  ...
                  specialize (H0 radix2 custom_emax).
                  pose proof bpow_gt_0.
                  specialize (H4 radix2 custom_emin).
                  psatz R.
                Qed.
                intros.
                pose proof ubMax as ubMaxProof.
                specialize (ubMaxProof ub1 ub2 resultGe0).
                specialize (plusRoundingTruth3 ubMaxProof floatMaxBound1 x1ub x2ub).
                pose proof plusRoundingTruth2 as plusRoundingTruth2.
                specialize (plusRoundingTruth2 x x0 x1 x2 floatToRealProof1 floatToRealProof2 plusRoundingTruth3).
                destruct plusRoundingTruth2.
                assert (floatPlusRoundRel := H).
                assert (resultFiniteProof := H1).
                clear H H1.
                apply validFloat2 in floatToRealProof1.
                apply validFloat2 in floatToRealProof2.
                rewrite <- floatToRealProof1 in floatPlusRoundRel. 
                rewrite <- floatToRealProof2 in floatPlusRoundRel.
                rewrite <- plusResultStmt in floatPlusRoundRel.
                unfold B2R in floatPlusRoundRel.
                rewrite floatPlusRoundRel.
               
                (* relative_error*)
                pose proof relative_error as relative_error.
                specialize (relative_error radix2 (FLT_exp (3 - custom_emax - custom_prec) custom_prec) validFexpProof (custom_emin)%Z custom_prec precThm (round_mode mode_NE) (valid_rnd_N choiceDef) (x1 + x2)%R).
                Lemma absHelper2: forall x y, (x<0)%R -> (x<=0-y)%R -> (y<=(Rabs x))%R.
                  intros. unfold Rabs in *. destruct Rcase_abs; psatz R. Qed.
                Lemma negResInf : forall ub1 ub2 x1 x2, (x1 <= ub1 -> x2 <= ub2 -> ub1 + ub2 <0 -> x1 + x2 < 0)%R.
                  intros. 
                  psatz R. Qed.
                intros.
                
                pose proof absHelper2 as absProof.
                pose proof negResInf as negResInf.
                specialize (negResInf ub1 ub2 x1 x2 x1ub x2ub resultGe0).
                specialize (absProof (x1+x2)%R floatMin negResInf floatMinCase3).
                specialize (relative_error absProof).
                (* relative_error *)
                remember ( round radix2 (FLT_exp (3 - custom_emax - custom_prec) custom_prec)
      (round_mode mode_NE) (x1 + x2)) as roundedValue.
                unfold Rabs in relative_error.
                clear absProof floatPlusRoundRel resultFiniteProof ubMaxProof orExtra plusRoundingTruth3 plusResultStmt H0 floatMaxBound1 HeqroundedValue.
                pose proof errorGt0.
                pose proof errorLessThan1.
                unfold error in *.
                destruct Rcase_abs; destruct Rcase_abs;
              ...
              {
                destruct (Rle_lt_or_eq_dec (x1+x2)%R 0).
                {
                  psatz R.
                }
                {
                  apply Rnot_le_gt in n.
                  generalize (el2 radix2 (-(x1+x2))%R (ln_beta radix2 (-(x1+x2))%R)%R).
                  intros.
                  apply Ropp_gt_lt_0_contravar in r0.
                  assert (resultNe0 := r0).
                  apply gt0ImpNE0 in resultNe0.
                  specialize (H resultNe0).
                  pose proof subNormal as subNormalProof.
                  specialize (subNormalProof (-(x1+x2))%R r0).
                  Lemma negFloatMinRel : forall x, (x > 0 -floatMin -> -x < floatMin)%R.
                    intros. psatz R. Qed.
                  pose proof negFloatMinRel as negFloatMinRel.
                  specialize (negFloatMinRel (x1+x2)%R n).
                  specialize (subNormalProof negFloatMinRel).
                  remember (ln_beta radix2 (- (x1 + x2))) as ex.
                  
                  Lemma gtNegFloatMinImpGtFloatMax : forall x, x < 0 -> x > 0 - floatMin -> Rabs x < floatMax 

                  SearchAbout (_ > 0 -> _ <> 0)%R.
                 SearchAbout (_ < 0  ->  _ > 0)%R.
                  
              Lemma subNormal2: forall x:R,  (x < R0)%R -> (x < 0-floatMin)%R ->  (ln_beta radix2 (0-x <= custom_emin)%Z.
                intros.
                unfold floatMin in *.
                assert (H1 := H).
                pose proof ln_beta_le_bpow.
              ...
              Qed.
                z3 solve_dbg.
                pose proof round_opp as round_opp.
                specialize (round_opp radix2 (FLT_exp (3 - custom_emax - custom_prec) custom_prec) (round_mode mode_NE) (0-(x1+x2))%R).
                Lemma zeroMin : forall x1 x2, (- (0 - (x1 + x2)))%R = (x1 + x2)%R.
                  intros.
                  psatz R.
                  Qed.
                unfold Zrnd_opp in *. 
                rewrite zeroMin in round_opp.
          ...
          {
             {
               destruct H0.
               {
                 ...
                 assert (expr1Bound := H1).
                 assert (expr2Bound := H3).
                 assert (floatMinCase1 := H7).
                 assert (floatMinCase2 := H8).
                 assert (floatMaxBound1 := H9).
                 assert (floatMaxBound2 := H10).
                 assert (floatMaxBound3 := H11).
                 assert (floatMaxBound4 := H13).
                 assert (resultGe0 := H2).
                 clear H4 H2 H1 H3 H H5 H0 H6 IHexpr1 IHexpr2.
                 unfold Semantics.eval_comp in *.
                 simpl in H7,H8,H9,H11,H10,H13.
                 simpl in floatMaxBound2.
                 simpl in floatMaxBound3.
                 simpl in floatMaxBound4.
                 simpl in floatMinCase1.
                 simpl in floatMinCase2.
                 simpl in expr1Bound.
                 simpl in expr2Bound.
                 ...
                 remember (eval_term (ub x5) (hd tr) (hd (tl tr))) as ub2.
                 remember (eval_term (ub x4) (hd tr) (hd (tl tr))) as ub1.
                 clear Hequb1 Hequb2 Heqlb1 Heqlb2 H7 H8 H9 H10 H11 H13.
                 
                 simpl in minusResultStmt.
                 Lemma simplBound8Proof : forall ub1 lb1 lb2 ub2 floatMax x1 x2 x x0 f, 
                 f = Bminus custom_prec custom_emax custom_precGt0
                      custom_precLtEmax custom_nan mode_NE x x0
                      -> Some x1 = floatToReal x -> Some x2 = floatToReal x0 -> (lb1 - ub2 < 0 - floatMin)%R-> (ub1 - lb2 > floatMin)%R -> (ub1 - lb2 < floatMax)%R -> (ub2 - ub1 < floatMax)%R -> (ub2 - lb1 < floatMax)%R -> (lb1 - lb2 < floatMax)%R -> (lb1 <= x1 <= ub1)%R -> (lb2 <= x2 <= ub2)%R -> (ub1 >= lb2)%R -> (ub2 - ub1 < floatMax)%R -> (ub2 >= lb1)%R -> ((lb1 - ub2) * (1+error) <= B2R _ _ f <= (ub1-lb2) * (1+error))%R.
                   intros.
                   destruct (Rle_dec (x1 - x2) floatMin).
                   {
                     pose proof errorGt0.
                     destruct (Rge_dec (x1 - x2) (0 - floatMin)).
                     {
                       z3 solve_dbg.
                       psatz R.
                     }
                     {
                       ...
                       }
                       {
                         ...
                         }
                         {
                           pose proof relative_error. 
                           pose proof Bminus_correct.
                           specialize (H14 custom_prec custom_emax).
                         }
                       }
                       destruct x0.
                       {
